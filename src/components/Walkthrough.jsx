    ...
    <section id="walkthrough" className="py-24 px-6 bg-obsidian">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {areas.map((area, index) => (
          <div key={index} className="relative aspect-square overflow-hidden group border border-white/10 rounded-xl">
            
            {/* Layer 1: Background Image */}
            <img 
              src={area.img} 
              alt={area.name}
              className="absolute inset-0 w-full h-full object-cover transition-all duration-700 grayscale-[0.5] brightness-[0.4] group-hover:grayscale-0 group-hover:brightness-100 group-hover:scale-110"
            />

            {/* Layer 2: Gradient Overlays */}
            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent opacity-80 z-10" />
            <div className="absolute inset-0 bg-violet/40 opacity-0 group-hover:opacity-20 transition-opacity duration-500 z-10" />
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 z-20 text-center">
              ...
              <p className="text-gray-200 text-sm font-medium opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0 duration-500 max-w-[80%] drop-shadow-md">
                {area.desc}
              </p>
            </div>
            
            {/* Layer 4: Animated Bottom Border */}
            <div className="absolute bottom-0 left-0 w-0 h-1.5 bg-gold transition-all duration-500 group-hover:w-full z-30" />
          </div>
        ))}
      </div>
    </section>
